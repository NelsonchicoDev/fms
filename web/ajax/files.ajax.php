		...
		if($folder->status == 200){
			...
			if($this->folder == 2){
				...
				$s3 = new S3Client([	
				    ...
				    'region'  => 'us-east-2',
				    'credentials' => [
				    	'key' => explode("|",$folder->keys_folder)[0],
				    	'secret' => explode("|",$folder->keys_folder)[1]
				    ]
				]);
				...
				if($result['@metadata']['statusCode'] == 200){
					...
					$fields = array(
						...
						"type_file" => $this->file["type"],
						"size_file" => $this->file["size"],
						"link_file" => $result['@metadata']['effectiveUri'],
						"date_created_file" => date("Y-m-d")
					);
					...
					$response = array(
						...
						"status" => 404,
						"error" => "Error al intentar subir el archivo a AWS"
			...
			if($this->folder == 4){
				...
				/*=============================================
				Validar que el archivo si sea de video
				=============================================*/

				if(explode("/",$this->file["type"])[0] != "video"){

					$response = array(

						"status" => 404,
						"error" => "Solo para Vimeo se permiten archivos de Video"
				...
				$lib = new Vimeo(explode("|",$folder->keys_folder)[0], explode("|",$folder->keys_folder)[1], explode("|",$folder->keys_folder)[2]);
			...
			if($this->folder == 5){
				...
				if(explode("/",$this->file["type"])[0] != "image"){

					$response = array(

						"status" => 404,
						"error" => "Solo para Mailchimp se permiten archivos de Imagen"
				...
				$client->setConfig([
				    'apiKey' => explode("|",$folder->keys_folder)[0],
				    'server' => explode("|",$folder->keys_folder)[1],
				]);

				$imageData = file_get_contents($this->file["tmp_name"]);
	...
	public function deleteFile(){
		...
		if($this->idFolderDelete == 2){
			...
			$s3 = new S3Client([	
			    ...
			    'region'  => 'us-east-1',
			    'credentials' => [
			    	'key' => explode("|",$getFolder->keys_folder)[0],
			    	'secret' => explode("|",$getFolder->keys_folder)[1]
			    ]
			]);
		...
		/*=============================================
		Eliminando archivos de Vimeo
		=============================================*/

		if($this->idFolderDelete == 4){

			$lib = new Vimeo(explode("|",$getFolder->keys_folder)[0], explode("|",$getFolder->keys_folder)[1], explode("|",$getFolder->keys_folder)[2]);

			$arrayLink = explode("/",$getFile->link_file);

			$uri = "/videos/".end($arrayLink);
			$lib->request($uri, array(), 'DELETE');

		}


		/*=============================================
		Eliminando archivos de Mailchimp
		=============================================*/
		...
		if($this->idFolderDelete == 5){
			...
			$client = new MailchimpMarketing\ApiClient();
			$client->setConfig([
			    'apiKey' => explode("|",$getFolder->keys_folder)[0],
			    'server' => explode("|",$getFolder->keys_folder)[1]
			]);

			$response = $client->fileManager->deleteFile($getFile->id_mailchimp_file);
